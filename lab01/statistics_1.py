...
def extract_features(word):
    return {
        ...
        "capitalization": (
            "all_caps"
            if word.isupper()
            else "start_cap" if word[0].isupper() else "lowercase"
        ),
...
def analyze_entity_word_structure(xml_path):
    ...
    for sentence in sentences:
        ...
        for entity in entities:
            ...
            preceding_word = (
                tokens[start_index - 1] if start_index and start_index > 0 else ""
            )
            following_word = (
                tokens[end_index + 1]
                if end_index and end_index < len(tokens) - 1
                else ""
            )
...
def print_statistics(stats):
    for entity_type, data in stats.items():
        ...
        print(
            f"Durchschnittliche Länge: {sum(data['lengths']) / len(data['lengths']) if data['lengths'] else 'N/A'}"
        )
